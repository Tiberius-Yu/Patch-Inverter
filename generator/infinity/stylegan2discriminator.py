...
from generator.infinity.ops import *
...
class StyleGan2Discriminator(nn.Module):
    ...
    def __init__(self, config, no_adds_on=False):
        ...
        if no_adds_on:
            ...
            self.use_coord_ac = hasattr(self.config.train_params,
                                        "coord_use_ac") and self.config.train_params.coord_use_ac
            self.use_coord_pd = hasattr(self.config.train_params,
                                        "coord_use_pd") and self.config.train_params.coord_use_pd
        ...
        self.stddev_group = self._smallest_divisor_larger_than(
            config.train_params.batch_size, start=4)
        self.stddev_feat = 1
        ...
        if self.use_coord_ac:
            if hasattr(config.train_params,
                       "coord_ac_categorical") and config.train_params.coord_ac_categorical:
                ...
                self.coord_linear = nn.Sequential(
                    EqualLinear(
                        ...
                        activation='fused_lrelu'),
                    EqualLinear(
                        linear_ch, config.train_params.coord_num_dir *
                        config.train_params.coord_vert_sample_size),
                )
            else:
                self.coord_linear = nn.Sequential(
                    EqualLinear(
        ...
        if self.use_coord_pd:
            if hasattr(config.train_params,
                       "coord_pd_hori_only") and config.train_params.coord_pd_hori_only:
                self.coord_proj_dim = config.train_params.coord_num_dir - 1
            else:
                self.coord_proj_dim = config.train_params.coord_num_dir
            self.coord_proj = nn.Sequential(
                EqualLinear(self.coord_proj_dim, linear_ch, activation='fused_lrelu'),
