...
from generator.infinity.coord_handler import CoordHandler
from generator.infinity.infinity_ops import *
from generator.infinity.losses import calc_path_lengths
from generator.infinity.dataset import DictTensor
...
def create_fusion_styles(cur_fusion_map, styles, config):
    ...
    fused_style = torch.zeros(bs, config.train_params.global_latent_dim, cur_height, cur_width)
...
class ConditionalBlock(nn.Module):
    ...
    def calibrate_spatial_shape(self,
        ...
        return self.conv.calibrate_spatial_shape(
            feature, direction, padding_mode=padding_mode, verbose=verbose, pin_loc=pin_loc)
    ...
    def forward(self, x, cond, coords, noise=None, test_ids=None, calc_flops=False):
        ...
        return self.conv(
            x, cond, noise=noise, coords=coords, test_ids=test_ids, calc_flops=calc_flops)
...
class ImplicitFunction(nn.Module):
    ...
    def __init__(self, config):
        ...
        convs = [ConditionalBlock(idx=i, config=config) for i in range(num_res_block)]
    ...
    def forward(self,
        ...
        for i, conv in enumerate(self.conv_stack):
            ...
            h, cur_flops = conv(
                h, cur_cond, coords, noise=cur_noise, test_ids=test_ids, calc_flops=calc_flops)
            flops += cur_flops
...
class StructureSynthesizer(nn.Module):
    ...
    def __init__(self, config):
        ...
        if hasattr(self.config.train_params,
                   "diversity_z_w") and self.config.train_params.diversity_z_w != 0:
            self.use_div_z_loss = True
        else:
            self.use_div_z_loss = False

        if hasattr(self.config.train_params,
                   "diversity_angular") and self.config.train_params.diversity_angular != 0:
    ...
    def calibrate_spatial_shape(self,
                                ...
                                verbose=False,
                                pin_loc=None):
        return self.implicit_model.calibrate_spatial_shape(
            feature, direction, padding_mode=padding_mode, verbose=verbose, pin_loc=pin_loc)
    ...
    def forward(
        ...
        if self.config.train_params.ss_coord_all_layers:
            ...
            local_latent = torch.cat([local_latent, coords], 1)
...
class TextureSynthesizer(nn.Module):
    ...
    def __init__(self, config):
        ...
        is_styleGAN2_baseline = (
            hasattr(config.train_params, "styleGAN2_baseline")
            and config.train_params.styleGAN2_baseline)
        ...
        if config.train_params.ts_no_zero_pad:
            ...
            assert hasattr(self.config.train_params,
                           "styleGAN2_baseline") and self.config.train_params.styleGAN2_baseline
            blur_kernel = [1, 3, 3, 1]
        ...
        if g_output_res == 101 and config.train_params.ts_input_size == 11:
            ...
            raise NotImplementedError(
                " Not yet designed arch for G input size {} and output res {}".format(
    ...
    def mean_latent(self, n_latent):
        latent_in = torch.randn(n_latent, self.global_latent_dim, device=self.const_z.input.device)
        ...
        if style_fusion_map is not None:
            if wplus_styles is None:
                ...
                for i in range(len(style_fusion_map)):
                    cur_wplus_style_centers = [s[:, i] for s in wplus_styles]
                    cur_fusion_map = create_fusion_styles(style_fusion_map[i],
                                                          cur_wplus_style_centers, self.config)
                ...
                for to_rgbs_spec in self.to_rgbs_specs:
                    ...
                    cur_wplus_style_centers = [s[:, tgt_idx] for s in wplus_styles]
                    fused_style = create_fusion_styles(cur_fusion_map, cur_wplus_style_centers,
                                                       self.config)
                    torgbs_fused_styles.append(fused_style)
        """
...
class InfinityGanGenerator(nn.Module):
    ...
    def __init__(self, config):
        ...
        if hasattr(config.train_params,
                   "styleGAN2_baseline") and config.train_params.styleGAN2_baseline:
        ...
        if hasattr(self.config.train_params,
                   "diversity_angular") and self.config.train_params.diversity_angular:
            self.use_angular_div = True
        else:
            self.use_angular_div = False
    ...
    def forward(
        ...
        if (local_latent is not None) and (self.structure_synthesizer is not None):
            structure_latent, coords, ac_coords, flops_ss = self.structure_synthesizer(
                global_latent=
                global_latent,  # if isinstance(global_latent, list) else global_latent[:, 0],
        ...
        if early_return_structure_latent:
            return DictTensor(structure_latent=structure_latent, coords=coords)
        """
        Texture Synthesizer
        """
    ...
    def texture_generate(self, global_latent, structure_latent, noises):

        output = self.texture_synthesizer(
            global_latent=global_latent, structure_latent=structure_latent, noises=noises)
        return output
