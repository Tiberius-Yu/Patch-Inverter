    ...
    def forward(self, input):
        if self.no_zero_pad:
            ...
            out = F.conv_transpose2d(
                input.view(B * C, 1, H, W), self.kernel.unsqueeze(0).unsqueeze(1), stride=2)
            _, _, nH, nW = out.shape
...
class EqualLinear(nn.Module):
    ...
    def get_flops(self, input):
        ...
        flops += np.prod(self.weight.shape) + np.prod(
            self.weight.shape)  # linear weight, includes self.scale
...
class ModulatedConv2d(nn.Module):
    ...
    def __init__(
        ...
        if upsample:
            ...
            if no_zero_pad:
                self.dirty_rm_size = (
                    pad0, pad1)  # The dirty pixels going to be cut-off from features during runtime
            ...
            if no_zero_pad:
                self.dirty_rm_size = (
                    0, 0)  # The dirty pixels going to be cut-off from features during runtime
                self.blur = Blur(blur_kernel, pad=(0, 0))
            else:
                self.dirty_rm_size = (0, 0)
    ...
    def calc_in_spatial_size(self, out_spatial_size, verbose=False):
        ...
        if self.upsample:
            ...
            in_spatial_size = round_to_even(out_spatial_size + 1 + self.dirty_rm_size[0] +
                                            self.dirty_rm_size[1]) // 2
            if verbose:
                print("\t\t {} = ({} + {} + {}) // 2".format(in_spatial_size, out_spatial_size,
                                                             self.dirty_rm_size[0],
                                                             self.dirty_rm_size[1]))
            ...
            if verbose:
                print("\t\t {} = {} + {} + {}".format(in_spatial_size, out_spatial_size,
                                                      self.dirty_rm_size[0], self.dirty_rm_size[1]))
    ...
    def calc_out_spatial_size(self, in_spatial_size):
        ...
        """
        if self.upsample:
            out_spatial_size = in_spatial_size * 2 - 1 - self.dirty_rm_size[0] - self.dirty_rm_size[
                1]
        elif self.downsample:
            assert False, "Never used, not sure if exists any problem here."
    ...
    def calibrate_spatial_shape(self,
                                feature,
                                direction,
        ...
        if direction == "forward":
            if self.upsample:
                ...
                if pin_loc is not None:
                    ...
                    pin_loc_to_old_center = [pin_loc[0] - old_center[0], pin_loc[1] - old_center[1]]
                    pin_loc_to_new_center = [
                        pin_loc_to_old_center[0] * 2, pin_loc_to_old_center[1] * 2
                    ]
                    pin_loc = [
                ...
                if (self.padding == 0):
                    ...
                    feature = feature[:, :, self.dirty_rm_size[0]:-self.dirty_rm_size[0],
            ...
            if verbose:
                if pin_loc is None:
                    ...
                    print(
                        " [*] Calibration: ({}, {}) => ({}, {}) ; Pin: ({}, {}) => ({}, {})".format(
                            h, w, feature.shape[2], feature.shape[3], prev_pin_loc[0],
                            prev_pin_loc[1], pin_loc[0], pin_loc[1]))
            ...
            if self.upsample:
                ...
                if (self.dirty_rm_size != (0, 0)):
                    feature = F.pad(
                        feature,
                        (self.dirty_rm_size[1], self.dirty_rm_size[1], self.dirty_rm_size[0],
                         self.dirty_rm_size[0]),  # (L, R, T, B)
                        mode=padding_mode)
                ...
                feature = F.interpolate(
                    feature, size=recovered_input_size, mode="bilinear", align_corners=True)

                if pin_loc is not None:
                    # [d]
                    pin_loc = [
                        pin_loc[0] + self.dirty_rm_size[0], pin_loc[1] + self.dirty_rm_size[1]
                    ]
                    # [c][b] skip
                    # [a]
            ...
            raise NotImplementedError(
                "Unknown direction {} (valid: 'forward' or 'backward')".format(direction))
    ...
    def forward(self, input, style, coords=None, calc_flops=False):
        ...
        if style.ndim == 2:
            ...
            if self.upsample:
                ...
                if self.no_zero_pad:
                    out = out[:, :, 1:-1,
                              1:-1]  # Clipping head and tail, which involves zero padding
                _, _, height, width = out.shape
                out = out.view(batch, self.out_channel, height, width)
                out = self.blur(out)
            ...
            if self.upsample:
                ...
                if self.demodulate:
                    demod = F.interpolate(
                        demod, size=(height, width), mode="bilinear", align_corners=True)
                    out = out * demod
                out = self.blur(out)
            elif self.downsample:
                ...
                if self.demodulate and (self.padding == 0):
                    demod = demod[:, :, self.dirty_rm_size[0]:-self.dirty_rm_size[0],
                                  self.dirty_rm_size[1]:-self.dirty_rm_size[1]]
                    out = out * demod
            ...
            for test_id in test_ids:
                ...
                if test_id not in self.testing_noise:
                    ...
                    if (cache_h < height) or (cache_w < width):
                        ...
                        new_testing_noise[:, pad_h:pad_h + cache_h,
                                          pad_w:pad_w + cache_w] = cur_testing_noise
                ...
                batch_cur_testing_noise.append(cur_testing_noise[:, pad_h:pad_h + height,
                                                                 pad_w:pad_w + width])
...
class StyledConv(nn.Module):
    ...
    def calibrate_spatial_shape(self,
                                ...
                                verbose=False,
                                pin_loc=None):
        return self.conv.calibrate_spatial_shape(
            spatial_latent, direction, padding_mode=padding_mode, verbose=verbose, pin_loc=pin_loc)
...
class Skip(nn.Module):
    ...
    def __init__(self, in_channel, downsample=True, blur_kernel=[1, 2, 1], no_zero_pad=False):
        super().__init__()
        self.no_zero_pad = no_zero_pad

        if downsample:
            self.downsample = nn.Conv2d(
                256, 256, kernel_size=1, stride=2, padding=4, padding_mode="replicate")

        self.conv = nn.Conv2d(
            in_channels=in_channel * 4, out_channels=256, kernel_size=1, stride=1, padding=1)
        self.bias = nn.Parameter(torch.zeros(1, 256, 1, 1))
    ...
    def forward(self, input, skip=None, calc_flops=False):
        flops = 0
        B, C, H, W = input.shape
        h1 = input.clone()
        h2 = input.clone()
        h3 = input.clone()
        h1[:, :, :H - 1, :W - 1] = input[:, :, 1:, 1:]
        h2[:, :, :H - 1] = input[:, :, 1:]
        h3[:, :, :, :W - 1] = input[:, :, :, 1:]
        h_final = torch.cat([input, h1, h2, h3], dim=1)[:, :, 1:, 1:]
        out = self.conv(h_final)
        out = out[:, :, 1:, 1:]
