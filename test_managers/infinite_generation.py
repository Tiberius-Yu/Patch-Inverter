...
class InfiniteGenerationManager(BaseTestManager):
    ...
    def task_specific_init(self, output_size=None):
        ...
        if output_size is None:
            self.target_height = self.img_size[1]
            self.target_width = self.img_size[0]
        else:
            self.target_height, self.target_width = output_size

        self._init_starting_points()
        self.noise_heights = self.outfeat_step_sizes * (self.num_steps_h -
                                                        1) + self.outfeat_sizes_list
        self.noise_widths = self.outfeat_step_sizes * (self.num_steps_w -
                                                       1) + self.outfeat_sizes_list

        self.gen_from_inv_stats = False

        self.full_local_latent_shape = (
        ...
        print(self.full_local_latent_shape)

    def run_next(self,
                 save=True,
                 write_gpu_time=False,
                 inv_records=None,
                 inv_placements=None,
                 calc_flops=False,
                 disable_pbar=False,
    ...
    def create_vars(self, global_latent, local_latent, inv_records=None, inv_placements=None):
        mixing = False
        ...
        # Can be reused
        meta_img = torch.empty(self.config.train_params.batch_size, 3, int(self.meta_height),
        ...
        meta_coords = self.coord_handler.sample_coord_grid(local_latent, is_training=False)
    ...
    def generate(self,
                 testing_vars,
                 tkinter_pbar=None,
                 update_by_ss_map=None,
                 update_by_ts_map=None,
                 write_gpu_time=False,
                 calc_flops=False,
                 disable_pbar=False):
        ...
        idx_tuples = list(
            iter_product(
                range(self.start_pts_mesh_z.shape[0]), range(self.start_pts_mesh_z.shape[1])))
        ...
        for iiter, (idx_x, idx_y) in enumerate(pbar):
            ...
            outfeat_x_st = [
                start_pts_mesh[idx_x, idx_y, 0] for start_pts_mesh in self.start_pts_mesh_outfeats
            ]
            outfeat_y_st = [
                start_pts_mesh[idx_x, idx_y, 1] for start_pts_mesh in self.start_pts_mesh_outfeats
            ]
            outfeat_x_ed = [
            ...
            for i, (fx_st, fy_st, fx_ed, fy_ed) in enumerate(
                ...
                noises.append(testing_vars.noises[i][:, :, fx_st:fx_ed,
            ...
            cur_local_latent = testing_vars.local_latent[:, :, zx_st:zx_ed,
                                                         zy_st:zy_ed].to(self.device)
            ...
            exec_time, flops = self.maybe_parallel_inference(
                ...
                return_exec_time=write_gpu_time,
                calc_flops=calc_flops)
        ...
        if calc_flops:
            print(" [*] Total FLOPs: {} (SS {}, TS {})".format(
                self.pretty_print_flops(accum_flops_all), self.pretty_print_flops(accum_flops_ss),
    ...
    def _wrap_feature(self, feat, wrap_size, dim):
        ...
        if dim == 2:
            ...
            raise NotImplementedError(
                "I don't expect this function will be used other than spatial dims, but got {}.".
                format(dim))
    ...
    def _init_starting_points(self):
        ...
        self.num_steps_h = math.ceil(self.target_height / self.pixelspace_step_size)
        self.num_steps_w = math.ceil(self.target_width / self.pixelspace_step_size)
        self.small_num_steps_h = \
            math.ceil((self.target_height - self.outfeat_sizes_list[-1]) / self.pixelspace_step_size) + test_meta_extra_pad
        self.small_num_steps_w = \
        ...
        self.meta_width = self.pixelspace_step_size * (self.num_steps_w -
                                                       1) + self.outfeat_sizes_list[-1]
        self.small_meta_height = self.pixelspace_step_size * (self.small_num_steps_h -
                                                              1) + self.outfeat_sizes_list[-1]
        self.small_meta_width = self.pixelspace_step_size * (self.small_num_steps_w -
                                                             1) + self.outfeat_sizes_list[-1]

    def compose_inv_root(self):
        return os.path.join("./logs/", self.config.var.exp_name, "test",
                            self.config.task.prev_inv_config, "stats")
