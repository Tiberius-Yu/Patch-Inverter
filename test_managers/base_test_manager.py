...
class BaseTestManager():
    ...
    def __init__(self, g_ema, config, logdir):
        self.g_ema = g_ema.eval()
        self.img_size = (config['resolution'], config['resolution'])
        self.config = config['model']
        config = config['model']
        self.save_root = logdir
        ...
        print(f"{self.enable_parallel_batching=}")
        ...
        self.infeat_sizes_list = self._get_feature_sizes(
            side="input", output_size=self.outfeat_sizes_list[-1], include_ss=False)
        self.in_size_doubled_list = self._get_feature_sizes(
            side="input", output_size=self.outfeat_sizes_list_doubled[-1], include_ss=False)
        ...
        # FIXME: Directly using the 32. Need to be fixed when Infinity reconstruction
        # torch.roll will alleviate the 32 problem to 48 :)
        # seems wrong in 32 to 48
        self.pixelspace_step_size = 48
        # self.pixelspace_step_size = (self.outfeat_sizes_list[-1] //
        #                              pixelspace_disp_unit) * pixelspace_disp_unit
        ...
        if config.train_params.training_modality == "patch":
            ...
            raise ValueError("Unknown training modality {}".format(
                config.train_params.training_modality))
    ...
    def _get_feature_sizes(self, side=None, multiplier=1, output_size=None, include_ss=False):
        ...
        if side == "input":
            ...
            size_list = self.g_ema_module.calc_out_spatial_size(
                self.config.train_params.ts_input_size * multiplier,
    ...
    def maybe_parallel_inference(self,
                                 testing_vars,
                                 g_ema_kwargs=None,
                                 index_tuple=None,
                                 flush=False,
                                 return_exec_time=False,
                                 calc_flops=False):
        exec_time = -1
        ...
        if self.enable_parallel_batching:
            ...
            if flush and len(self.parallel_stack) == 0:
                ...
                for i, d in enumerate(self.parallel_stack):
                    ...
                    testing_vars.meta_img[:, :, img_x_st:img_x_ed,
