...
class InversionManager(BaseTestManager):
    ...
    def task_specific_init(self):
        ...
        self.meta_height = self.pixelspace_step_size * (self.num_steps_h -
                                                        1) + self.outfeat_sizes_list[-1]
        self.meta_width = self.pixelspace_step_size * (self.num_steps_w -
                                                       1) + self.outfeat_sizes_list[-1]
        ...
        if hasattr(self.config.task, "img_dir"):
            ...
            raise ValueError(
                "Either `img_dir` or `data_split` should be specified in the test_config!")
        self.dataloader = iter(
            data.DataLoader(
                dataset,
                batch_size=self.config.task.batch_size,
                sampler=data_sampler(
                    dataset, shuffle=False, init_index=self.config.task.init_index),
                drop_last=False,
            ))
        ...
        global_latents = self.latent_sampler.sample_global_latent(
            n_random_samples, device=self.device)  # (B, 2, C)
        ...
        self.ts_styles_gau_invcov = torch.from_numpy(prior_gau_invcov).float().to(
            self.device)  # (C, C)
        """
        Loss
        """
        ...
        if self.config.task.write_tb_logs:
            self.log_dir = os.path.join("logs-inv", self.config.var.exp_name,
            ...
            if self.config.task.exp_suffix is not None:
                ...
                if hasattr(self.config.task,
                           "local_latent_renorm") and self.config.task.local_latent_renorm:
    ...
    def run_next(self, save=False, **kwargs):
        ...
        if len(kwargs) > 0:
            for k, v in kwargs.items():
                if v is not None:
                    print(" [Warning] task manager receives untracked arg {} with value {}".format(
                        k, v))
        """
        Get target image
        """
        ...
        ss_global_latents = self.latent_sampler.sample_global_latent(
            cur_batch_size, mixing=False, device=self.device, requires_grad=True)
        ...
        if hasattr(self.config.task, "future_placement"):
            ...
            coords = self.coord_handler.sample_coord_grid(
                ss_local_latents, is_training=False
            )  # is_training=False creates zero-centered coords without using cache.
        if self.config.task.learned_coords:
        ...
        for iter_ in pbar:
            ...
            ts_styles_rand_strength = self.ts_styles_gau_std.mean(
            ) * self.config.task.rand_styles_perturb * ramp_factor
            ts_styles_rand = self._rand_perturb_latents(ts_styles, progress_ratio,
                                                        ts_styles_rand_strength)

            # Randomly perturb ss_local, see if reduces overfitting for outward pixels
            if hasattr(self.config.task,
                       "rand_ss_local_perturb") and self.config.task.rand_ss_local_perturb > 0:
                ramp_factor = max(0, 1 -
                                  progress_ratio / self.config.task.rand_ss_local_perturb_ramp)**2
                ss_local_rand_strength = self.config.task.rand_ss_local_perturb * ramp_factor
                ss_local_latents_rnd = self._rand_perturb_latents(ss_local_latents, progress_ratio,
                                                                  ss_local_rand_strength)
            else:
                ss_local_latents_rnd = ss_local_latents
                ss_local_rand_strength = 0

            if hasattr(self.config.task,
                       "rand_ss_global_perturb") and self.config.task.rand_ss_global_perturb > 0:
                ramp_factor = max(0, 1 -
                                  progress_ratio / self.config.task.rand_ss_global_perturb_ramp)**2
                ss_global_rand_strength = self.config.task.rand_ss_global_perturb * ramp_factor
                ss_global_latents_rnd = self._rand_perturb_latents(ss_global_latents,
                                                                   progress_ratio,
                                                                   ss_global_rand_strength)
            else:
            ...
            losses = self.compute_losses(ss_local_latents_rnd, ss_global_latents_rnd, coords,
                                         ts_styles, ts_styles_gau, ts_noises, target_imgs,
                                         inv_results, inv_raw)
            ...
            for k, loss in losses.items():
                if torch.isnan(loss.mean()).item() or torch.isinf(loss.mean()).item():
                    ...
                    import pdb
                    pdb.set_trace()
            if torch.isnan(ss_global_latents.grad).any() or torch.isinf(
                    ss_global_latents.grad).any():
                import pdb
                pdb.set_trace()
            ts_optimizer.step()
            ss_optimizer.step()
            ...
            with torch.no_grad():
                ...
                inv_raw_comp = self._direct_compose(inv_raw, target_imgs, center_drift=center_drift)
            if self.config.task.eval and (iter_ % self.config.task.log_value_steps == 0
                                          or iter_ == self.config.task.n_steps - 1):
                with torch.no_grad():
                    if hasattr(self.config.task,
                               "eval_extra") and self.config.task.eval_extra == False:
                        eval_scores = {}
                    else:
                    ...
                    trait_imgs = self._create_trait_variations(ss_global_latents, ss_local_latents,
                                                               coords, ts_styles, ts_noises,
                                                               inv_raw, full_structure_latent_shape,
                                                               target_imgs, center_drift)

                self.maybe_logging(losses, eval_scores_reduced, logging_distrs, other_scalars,
                                   target_imgs, inv_results, inv_raw, inv_raw_anno, inv_raw_comp,
                                   gt_imgs, trait_imgs, iter_)
    ...
    def eval(self, inv_results, gt_imgs):
        ...
        eval_scores = {
            ...
            "outpaint_lpips":
            self.percept(outpaint_real, outpaint_fake).squeeze()
            if outpaint_real is not None else None,
        }
    ...
    def compute_losses(self, ss_local_latents, ss_global_latents, coords, ts_styles, ts_styles_gau,
                       ts_noises, target_imgs, inv_results, inv_raw):
        ...
        losses = {
            ...
            "ss_global_prior_loss":
            self.unit_gaussian_prior_loss(ss_global_latents[:, 0], mode="mean"),
        ...
        losses["ts_prior_loss"] = self.empirical_gaussian_prior_loss(ts_styles_gau,
    ...
    def empirical_gaussian_prior_loss(self, latents, empirical_mean, empirical_std,
                                      empirical_invcov):
        """
        ...
        for j in range(n_layers):
            ...
            # => (Bx1xC) @ (BxCx1)
            # => (Bx1x1)
            prior_loss += torch.sqrt(
                (mu_dist.transpose(1, 2) @ cur_empirical_invcov @ mu_dist).mean())

        return prior_loss / n_layers

    def empirical_gaussian_mean_loss(self, latents, empirical_mean, empirical_std,
                                     empirical_invcov):
        """
        [Let L be the number of latents in texture synthesizer]
        latents: shape (B, L, C)
    ...
    def maybe_logging(self, losses, eval_scores_reduced, logging_distrs, other_scalars, target_imgs,
                      inv_results, inv_raw, inv_raw_anno, inv_raw_comp, gt_imgs, trait_imgs, iter_):
    ...
    @torch.no_grad()
    def _create_trait_variations(self, ss_global_latents, ss_local_latents, coords, ts_styles,
                                 ts_noises, inv_raw, full_structure_latent_shape, target_imgs,
                                 center_drift):
        ...
        for v_id in range(NUM_VAR_PER_TRAIT):
            tmp_ss_global = self.latent_sampler.sample_global_latent(
                cur_batch_size, mixing=False, device=self.device, requires_grad=False)
            inter_output = self.g_ema(
        ...
        for v_id in range(NUM_VAR_PER_TRAIT):
            tmp_global_latents = self.latent_sampler.sample_global_latent(
                cur_batch_size, device=self.device)  # (B, 2, C)
            # tmp_ts_styles = \
        ...
        # Random TS noises
        for v_id in range(NUM_VAR_PER_TRAIT):
            tmp_ts_noises = [torch.randn_like(n, device=self.device) for n in ts_noises]
            inter_output = self.g_ema(
                global_latent=ss_global_latents,
                local_latent=ss_local_latents,
                override_coords=coords,
    ...
    def _calc_future_meta_shape(self, future_height, future_width):
        ...
        # To avoid edge-condition on the image edge, we generate an image slightly larger than
        # requested, then center-crop to the requested resolution.
        future_meta_height = self.pixelspace_step_size * (num_steps_h -
                                                          1) + self.outfeat_sizes_list[-1]
        future_meta_width = self.pixelspace_step_size * (num_steps_w -
                                                         1) + self.outfeat_sizes_list[-1]
